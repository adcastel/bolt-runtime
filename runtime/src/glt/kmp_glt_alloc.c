/*
 * kmp_abt_alloc.c -- private/shared dyanmic memory allocation and management
 */
...
#include "kmp_glt.h"
#include "kmp_glt_wrapper_malloc.h"
...
__kmp_initialize_bget( kmp_info_t *th )
{
    ...
    bectl( th, (bget_compact_t) 0, (bget_acquire_t) malloc, (bget_release_t) free,
           (bufsize) __kmp_global.malloc_pool_incr );
}
...
___kmp_allocate( size_t size KMP_SRC_LOC_DECL )
{
    ...
    KE_TRACE( 25, ( "-> __kmp_allocate( %d ) called from %s:%d\n", (int) size KMP_SRC_LOC_PARM ) );
        ptr = ___kmp_allocate_align( size, __kmp_global.align_alloc KMP_SRC_LOC_PARM );
