/*
 * kmp_abt_taskq.c -- TASKQ support for OpenMP.
 */
...
__kmp_taskq_eo( int *gtid_ref, int *cid_ref, ident_t *loc_ref )
{
    ...
    kmp_taskq_t       *tq   = & __kmp_global.threads[gtid] -> th.th_team -> t.t_taskq;
...
{
    int i;
    int nproc = __kmp_global.threads[global_tid] -> th.th_team -> t.t_nproc;
...
__kmpc_end_taskq(ident_t *loc, kmp_int32 global_tid, kmpc_thunk_t *taskq_thunk)
{
    ...
    tq = & __kmp_global.threads[global_tid] -> th.th_team -> t.t_taskq;
    nproc = __kmp_global.threads[global_tid] -> th.th_team -> t.t_nproc;
...
{
    kmpc_task_queue_t *queue;
    kmp_taskq_t       *tq  = &__kmp_global.threads[global_tid] -> th.th_team -> t.t_taskq;
    int                tid = __kmp_tid_from_gtid( global_tid );
