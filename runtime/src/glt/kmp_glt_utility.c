/*
 * kmp_abt_utility.c -- Utility routines for the OpenMP support library.
 */
...
__kmp_expand_file_name( char *result, size_t rlen, char *pattern )
{
    ...
    *end = 0;
    {
	int i;
	for(i = __kmp_global.xproc; i >= 10; i /= 10, ++default_cpu_width);
    }
	...
	while (*pattern != '\0' && pos < end) {
		...
		*pos++ = *pattern++;
		...
		case 'p':
		    {
			snp_result = KMP_SNPRINTF( pos, end - pos + 1, "%0*d", cpu_width, __kmp_global.dflt_team_nth );
