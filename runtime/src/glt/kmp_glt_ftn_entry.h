/*
 * kmp_abt_ftn_entry.h -- Fortran entry linkage support for OpenMP.
 */
...
#ifndef FTN_STDCALL
# error The support file kmp_glt_ftn_entry.h should not be compiled by itself.
#endif
...
FTN_GET_STACKSIZE( void )
{
        ...
        return (int)__kmp_global.stksize;
    #endif
}
...
FTN_GET_BLOCKTIME( void )
{
	...
	tid = __kmp_tid_from_gtid(gtid);
	thread = __kmp_thread_from_gtid(gtid);
        team = __kmp_global.threads[ gtid ] -> th.th_team;
...
FTN_GET_LIBRARY (void)
{
        ...
        return ((int) __kmp_global.library);
    #endif
}
...
xexpand(FTN_GET_NUM_PROCS)( void )
{
        ...
        return __kmp_global.avail_proc;
    #endif
}
...
{
    #ifndef KMP_STUB
        __kmp_global.generate_warnings = kmp_warnings_explicit;
    #endif
}
...
{
    #ifndef KMP_STUB
        __kmp_global.generate_warnings = FALSE;
    #endif
}
...
xexpand(FTN_GET_CANCELLATION)(void) {
    ...
    return __kmp_global.omp_cancellation;
#endif
}
...
FTN_GET_MAX_TASK_PRIORITY( void )
{
    ...
    return __kmp_global.max_task_priority;
#endif
}
#endif
