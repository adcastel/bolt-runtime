...
#include "kmp_glt_wrapper_getpid.h"
#include "kmp_glt_environment.h"
#include "kmp_glt_str.h"
#include "kmp_glt_settings.h"
#include "kmp_glt_i18n.h"
#include "kmp_glt_io.h"
#include "kmp_glt_error.h"
#include "kmp_glt_stats.h"
...
static void __kmp_register_library_startup( void );
...
static void __kmp_print_storage_map_gtid( int gtid, void *p1, void *p2, size_t size, char const *format, ...);
...
static void __kmp_check_mic_type()
{
    ...
    if( (cs_p->eax & 0xff0) == 0xB10 ) {
        __kmp_global.mic_type = mic2;
    } else if( (cs_p->eax & 0xf0ff0) == 0x50670 ) {
        __kmp_global.mic_type = mic3;
    } else {
...
{
    //printf("__kmp_do_serial_initialize\n");
    int i, gtid;
    int size;
    int status;
...
///    __kmp_validate_locks();
    ...
    __kmp_init_lock( & __kmp_global.global_lock     );
    __kmp_init_queuing_lock( & __kmp_global.dispatch_lock );
    __kmp_init_lock( & __kmp_global.debug_lock      );
    ...
    __kmp_init_bootstrap_lock( & __kmp_global.forkjoin_lock  );
    __kmp_init_bootstrap_lock( & __kmp_global.exit_lock      );
    __kmp_init_bootstrap_lock( & __kmp_global.tp_cached_lock );

    /* conduct initialization and initial setup of configuration */

    __kmp_runtime_initialize();
    //printf("despues de __kmp_runtime_initialize\n");
#if KMP_ARCH_X86_64 && (KMP_OS_LINUX || KMP_OS_WINDOWS)
    ...
    __kmp_global.dflt_team_nth_ub = __kmp_global.xproc;
    if( __kmp_global.dflt_team_nth_ub < KMP_MIN_NTH ) {
        __kmp_global.dflt_team_nth_ub = KMP_MIN_NTH;
    }
    if( __kmp_global.dflt_team_nth_ub > __kmp_global.sys_max_nth ) {
        __kmp_global.dflt_team_nth_ub = __kmp_global.sys_max_nth;
    }
    __kmp_global.max_nth = __kmp_global.sys_max_nth;

    // From "KMP_LIBRARY" part of __kmp_env_initialize()
    __kmp_global.library = library_throughput;
    // From KMP_SCHEDULE initialization
    __kmp_global.sched_static = kmp_sch_static_balanced;
    // AC: do not use analytical here, because it is non-monotonous
    //__kmp_global.sched_guided = kmp_sch_guided_iterative_chunked;
...
///    #endif // KMP_FAST_REDUCTION_BARRIER
///#if KMP_ARCH_X86_64 && (KMP_OS_LINUX || KMP_OS_WINDOWS)
///    if (__kmp_global.mic_type == mic2) { // KNC
...
///#endif
...
#ifdef KMP_DEBUG
    __kmp_global.env_checks = TRUE;   /* development versions have the extra checks */
#else
    __kmp_global.env_checks = FALSE;  /* port versions do not have the extra checks */
#endif
    ...
    //printf("despues __kmp_env_initialize\n");
    ...
    __kmp_global.threads_capacity = __kmp_initial_threads_capacity( __kmp_global.dflt_team_nth_ub );
    // Moved here from __kmp_env_initialize() "KMP_ALL_THREADPRIVATE" part
    __kmp_global.tp_capacity = __kmp_default_tp_capacity(__kmp_global.dflt_team_nth_ub, __kmp_global.max_nth, __kmp_global.allThreadsSpecified);
    ...
    KMP_DEBUG_ASSERT( __kmp_global.thread_pool == NULL );
    KMP_DEBUG_ASSERT( __kmp_thread_pool_insert_pt == NULL );
    KMP_DEBUG_ASSERT( __kmp_global.team_pool   == NULL );
    __kmp_global.thread_pool = NULL;
    __kmp_thread_pool_insert_pt = NULL;
    __kmp_global.team_pool   = NULL;

    /* Allocate all of the variable sized records */
    /* NOTE: __kmp_global.threads_capacity entries are allocated, but the arrays are expandable */
    /* Since allocation is cache-aligned, just add extra padding at the end */
    size = (sizeof(kmp_info_t*) + sizeof(kmp_root_t*))*__kmp_global.threads_capacity + CACHE_LINE;
    __kmp_global.threads = (kmp_info_t**) __kmp_allocate( size );
    __kmp_global.root    = (kmp_root_t**) ((char*)__kmp_global.threads + sizeof(kmp_info_t*) * __kmp_global.threads_capacity );

    /* init thread counts */
    KMP_DEBUG_ASSERT( __kmp_global.all_nth == 0 ); // Asserts fail if the library is reinitializing and
    KMP_DEBUG_ASSERT( __kmp_global.nth == 0 );     // something was wrong in termination.
    __kmp_global.all_nth = 0;
    __kmp_global.nth     = 0;

    /* setup the uber master thread and hierarchy */
    //printf("antes __kmp_register_root\n");
    gtid = __kmp_register_root( TRUE );
    //printf("despues __kmp_register_root\n");
...
///        __kmp_register_atfork();
    ...
    __kmp_global.init_counter ++;

    __kmp_global.init_serial = TRUE;

    if (__kmp_global.settings) {
        __kmp_env_print();
    }

#if OMP_40_ENABLED
    if (__kmp_global.display_env || __kmp_global.display_env_verbose) {
        __kmp_env_print_2();
    }
...
{
    //printf("__kmp_serial_initialize");
...
{
   // printf("__kmp_do_middle_initialize");
    int i, j;
    ...
    //
    // Save the previous value for the __kmp_global.dflt_team_nth so that
    // we can avoid some reinitialization if it hasn't changed.
    //
    prev_dflt_team_nth = __kmp_global.dflt_team_nth;

///#if KMP_AFFINITY_SUPPORTED
///    //
///    // __kmp_affinity_initialize() will try to set __kmp_global.ncores to the
///    // number of cores on the machine.
///    //
///    __kmp_affinity_initialize();
///
///    //
///    // Run through the __kmp_global.threads array and set the affinity mask
///    // for each root thread that is currently registered with the RTL.
///    //
///    for ( i = 0; i < __kmp_global.threads_capacity; i++ ) {
///        if ( TCR_PTR( __kmp_global.threads[ i ] ) != NULL ) {
///            __kmp_affinity_set_init_mask( i, TRUE );
///        }
///    }
///#endif /* KMP_AFFINITY_SUPPORTED */

    KMP_ASSERT( __kmp_global.xproc > 0 );
    if ( __kmp_global.avail_proc == 0 ) {
        __kmp_global.avail_proc = __kmp_global.xproc;
    }

    // If there were empty places in num_threads list (OMP_NUM_THREADS=,,2,3), correct them now
    j = 0;
    while ( ( j < __kmp_global.nested_nth.used ) && ! __kmp_global.nested_nth.nth[ j ] ) {
        __kmp_global.nested_nth.nth[ j ] = __kmp_global.dflt_team_nth = __kmp_global.dflt_team_nth_ub = __kmp_global.avail_proc;
        j++;
    }

    if ( __kmp_global.dflt_team_nth == 0 ) {
#ifdef KMP_DFLT_NTH_CORES
        //
        // Default #threads = #cores
        //
        __kmp_global.dflt_team_nth = __kmp_global.ncores;
        KA_TRACE( 20, ("__kmp_middle_initialize: setting __kmp_global.dflt_team_nth = __kmp_global.ncores (%d)\n",
          __kmp_global.dflt_team_nth ) );
#else
        //
        // Default #threads = #available OS procs
        //
        __kmp_global.dflt_team_nth = __kmp_global.avail_proc;
        KA_TRACE( 20, ("__kmp_middle_initialize: setting __kmp_global.dflt_team_nth = __kmp_global.avail_proc(%d)\n",
          __kmp_global.dflt_team_nth ) );
#endif /* KMP_DFLT_NTH_CORES */
    }

    if ( __kmp_global.dflt_team_nth < KMP_MIN_NTH ) {
        __kmp_global.dflt_team_nth = KMP_MIN_NTH;
    }
    if( __kmp_global.dflt_team_nth > __kmp_global.sys_max_nth ) {
        __kmp_global.dflt_team_nth = __kmp_global.sys_max_nth;
    }
    ...
    //
    KMP_DEBUG_ASSERT( __kmp_global.dflt_team_nth <= __kmp_global.dflt_team_nth_ub );

    if ( __kmp_global.dflt_team_nth != prev_dflt_team_nth ) {
        //
        // Run through the __kmp_global.threads array and set the num threads icv
        // for each root thread that is currently registered with the RTL
        // (which has not already explicitly set its nthreads-var with a
        // call to omp_set_num_threads()).
        //
        for ( i = 0; i < __kmp_global.threads_capacity; i++ ) {
            kmp_info_t *thread = __kmp_global.threads[ i ];
            if ( thread == NULL ) continue;
            if ( thread->th.th_current_task->td_icvs.nproc != 0 ) continue;

            set__nproc( __kmp_global.threads[ i ], __kmp_global.dflt_team_nth );
        }
    }
    KA_TRACE( 20, ("__kmp_middle_initialize: final value for __kmp_global.dflt_team_nth = %d\n",
      __kmp_global.dflt_team_nth) );

    /* we have finished middle initialization */
    TCW_SYNC_4(__kmp_global.init_middle, TRUE);
...
{
  //  printf("__kmp_middle_initialize");
...
{
    //    printf("__kmp_parallel_initialize");

    int gtid = __kmp_entry_gtid();      // this might be a new root

    /* synchronize parallel initialization (for sibling) */
    if( TCR_4(__kmp_global.init_parallel) ) return;
    __kmp_acquire_bootstrap_lock( &__kmp_global.initz_lock );
    if( TCR_4(__kmp_global.init_parallel) ) { __kmp_release_bootstrap_lock( &__kmp_global.initz_lock ); return; }
    ...
    /* jc: The lock __kmp_global.initz_lock is already held, so calling __kmp_serial_initialize
    ...
    */
    ...
    //
    __kmp_store_x87_fpu_control_word( &__kmp_global.init_x87_fpu_control_word );
    __kmp_store_mxcsr( &__kmp_global.init_mxcsr );
    __kmp_global.init_mxcsr &= KMP_X86_MXCSR_MASK;
    ...
    if ( __kmp_global.version ) {
        __kmp_print_version_2();
    }

    /* we have finished parallel initialization */
    TCW_SYNC_4(__kmp_global.init_parallel, TRUE);
...
__kmp_cleanup( void )
{
    ...
    if (TCR_4(__kmp_global.init_parallel)) {
        TCW_4(__kmp_global.init_parallel, FALSE);
    }

    if (TCR_4(__kmp_global.init_middle)) {
///#if KMP_AFFINITY_SUPPORTED
///        __kmp_affinity_uninitialize();
///#endif /* KMP_AFFINITY_SUPPORTED */
///        __kmp_cleanup_hierarchy();
        TCW_4(__kmp_global.init_middle, FALSE);
    }
    ...
    for ( f = 0; f < __kmp_global.threads_capacity; f++ ) {
        if ( __kmp_global.root[ f ] != NULL ) {
            __kmp_free( __kmp_global.root[ f ] );
            __kmp_global.root[ f ] = NULL;
        }
    }
    __kmp_free( __kmp_global.threads );
    // __kmp_global.threads and __kmp_global.root were allocated at once, as single block, so there is no need in
    // freeing __kmp_global.root.
...
///    #endif /* KMP_AFFINITY_SUPPORTED */
    ...
    KMP_INTERNAL_FREE( __kmp_global.nested_nth.nth );
    __kmp_global.nested_nth.nth = NULL;
    __kmp_global.nested_nth.size = 0;
    __kmp_global.nested_nth.used = 0;
...
__kmp_internal_begin( void )
{
    ...
    root = __kmp_global.threads[ gtid ]->th.th_root;
    KMP_ASSERT( KMP_UBER_GTID( gtid ));
...
__kmp_internal_end(void)
{
    ...
    for( i=0 ; i<__kmp_global.threads_capacity ; i++ )
        if( __kmp_global.root[i] )
            if( __kmp_global.root[i]->r.r_active )
                break;
    KMP_MB();       /* Flush all pending memory write invalidates.  */
    TCW_SYNC_4(__kmp_global.g.g_done, TRUE);

    if ( i < __kmp_global.threads_capacity ) {
...
//                    KMP_ASSERT( ! KMP_UBER_GTID( i ) );         // AC: there can be uber threads alive here
                    KMP_ASSERT( ! __kmp_global.root[i]->r.r_active );  // TODO: can they be active?
                }
            }
        #endif
        ...
        while ( __kmp_global.thread_pool != NULL ) {    // Loop thru all the thread in the pool.
            // Get the next thread from the pool.
            kmp_info_t * thread = (kmp_info_t *) __kmp_global.thread_pool;
            __kmp_global.thread_pool = thread->th.th_next_pool;
            // Reap it.
        ...
        // Reap teams.
        while ( __kmp_global.team_pool != NULL ) {     // Loop thru all the teams in the pool.
            // Get the next team from the pool.
            kmp_team_t * team = (kmp_team_t *) __kmp_global.team_pool;
            __kmp_global.team_pool = team->t.t_next_pool;
            // Reap it.
        ...
        TCW_SYNC_4(__kmp_global.init_common, FALSE);
...
{
    //printf("__kmp_internal_end_library 1 gtid_req=%d\n",gtid_req);
    ...
    if( TCR_4(__kmp_global.g.g_done) || !__kmp_global.init_serial ) {
        KA_TRACE( 10, ("__kmp_internal_end_library: already finished\n" ));
        return;
    }

    //printf("__kmp_internal_end_library 2\n");
    ...
    /* find out who we are and what we should do */
    {
        ...
        int gtid = (gtid_req>=0) ? gtid_req : __kmp_gtid_get_specific();
                    //printf("__kmp_internal_end_library 3 gtid=%d\n",gtid);
        ...
        if( gtid == KMP_GTID_SHUTDOWN ) {
            ...
            /* we don't know who we are, but we may still shutdown the library */
                                ...
                                //printf("__kmp_internal_end_library 31 gtid=%d\n",gtid);

            /* unregister ourselves as an uber thread.  gtid is no longer valid */
            if( __kmp_global.root[gtid]->r.r_active ) {
                                    //printf("__kmp_internal_end_library 32 gtid=%d\n",gtid);
    ...
    /* synchronize the termination process */
        //printf("__kmp_internal_end_library 4\n");

    __kmp_acquire_bootstrap_lock( &__kmp_global.initz_lock );
    //printf("__kmp_internal_end_library 5\n");
    ...
    /* We need this lock to enforce mutex between this reading of
       __kmp_global.threads_capacity and the writing by __kmp_register_root.
    ...
    */
    __kmp_acquire_bootstrap_lock( &__kmp_global.forkjoin_lock );
    //printf("__kmp_internal_end_library 6\n");

    /* now we can safely conduct the actual termination */
    __kmp_internal_end();
    //printf("__kmp_internal_end_library 7\n");

    __kmp_release_bootstrap_lock( &__kmp_global.forkjoin_lock );
    __kmp_release_bootstrap_lock( &__kmp_global.initz_lock );
    //printf("__kmp_internal_end_library 8\n");
    ...
    KA_TRACE( 10, ("__kmp_internal_end_library: exit\n" ) );
        ...
        if ( __kmp_debug_buf )
            __kmp_dump_debug_buffer();
    #endif
    //printf("__kmp_internal_end_library 9\n");

    __kmp_fini_allocator();

    /* last cleanup part */
    __kmp_global_destroy();
    //printf("__kmp_internal_end_library 10\n");
...
__kmp_internal_end_thread( int gtid_req )
{
    ...
    /* find out who we are and what we should do */
    {
        ...
        /* unregister ourselves as an uber thread.  gtid is no longer valid */
            ...
            if ( gtid >= 0 ) {
                __kmp_global.threads[gtid]->th.th_task_team = NULL;
            }
...
#if OMP_40_ENABLED
    KMP_DEBUG_ASSERT( __kmp_global.nested_proc_bind.used > 0 );
#endif /* OMP_40_ENABLED */
...
#if OMP_40_ENABLED
      __kmp_global.nested_proc_bind.bind_types[0],
#endif /* OMP_40_ENABLED */
...
__kmp_initialize_root( kmp_root_t *root )
{
    ...
    root->r.r_nested       = __kmp_global.dflt_nested;
    ...
    // TODO???: root_team->t.t_max_active_levels = __kmp_global.dflt_max_active_levels;
    root_team->t.t_sched.r_sched_type = r_sched.r_sched_type;
    root_team->t.t_sched.chunk        = r_sched.chunk;
    KA_TRACE( 20, ("__kmp_initialize_root: init root team %d arrived\n",
                    root_team->t.t_id ));
    ...
    hot_team =
        __kmp_allocate_team(
            root,
            1,                                                         // new_nproc
            __kmp_global.dflt_team_nth_ub * 2,                                // max_nproc
#if OMP_40_ENABLED
            __kmp_global.nested_proc_bind.bind_types[0],
#endif
            &r_icvs,
    ...
    // TODO???: hot_team->t.t_max_active_levels = __kmp_global.dflt_max_active_levels;
...
/*
   This function attempts to create free entries in __kmp_global.threads and __kmp_global.root, and returns the number of
   ...
   On all platforms, expansion is attempted on the arrays __kmp_global.threads_ and __kmp_global.root, with appropriate
   update to __kmp_global.threads_capacity.  Array capacity is increased by doubling with clipping to
    __kmp_global.tp_capacity, if threadprivate cache array has been created.
   Synchronization with __kmpc_threadprivate_cached is done using __kmp_global.tp_cached_lock.
...
__kmp_expand_threads(int nWish, int nNeed) {
    ...
    while(1) {
        ...
        //
        // Note that __kmp_global.threads_capacity is not bounded by __kmp_global.max_nth.
        // If __kmp_global.max_nth is set to some value less than __kmp_global.sys_max_nth
        // by the user via OMP_THREAD_LIMIT, then __kmp_global.threads_capacity may
        // become > __kmp_global.max_nth in one of two ways:
        //
        // 1) The initialization thread (gtid = 0) exits.  __kmp_global.threads[0]
        //    may not be resused by another thread, so we may need to increase
        //    __kmp_global.threads_capacity to __kmp_max_threads + 1.
        //
        ...
        //
        old_tp_cached = __kmp_global.tp_cached;
        __kmp_actual_max_nth = old_tp_cached ? __kmp_global.tp_capacity : __kmp_global.sys_max_nth;
        KMP_DEBUG_ASSERT(__kmp_actual_max_nth >= __kmp_global.threads_capacity);

        /* compute expansion headroom to check if we can expand and whether to aim for nWish or nNeed */
        nTarget = nWish;
        if(__kmp_actual_max_nth - __kmp_global.threads_capacity < nTarget) {
            ...
            if(nNeed) {
                ...
                /* best-effort */
                nTarget = __kmp_actual_max_nth - __kmp_global.threads_capacity;
                if(!nTarget) {
        ...
        minimumRequiredCapacity = __kmp_global.threads_capacity + nTarget;

        newCapacity = __kmp_global.threads_capacity;
        do{
            newCapacity =
        ...
        KMP_MEMCPY(newThreads, __kmp_global.threads, __kmp_global.threads_capacity * sizeof(kmp_info_t*));
        KMP_MEMCPY(newRoot, __kmp_global.root, __kmp_global.threads_capacity * sizeof(kmp_root_t*));
        memset(newThreads + __kmp_global.threads_capacity, 0,
               (newCapacity - __kmp_global.threads_capacity) * sizeof(kmp_info_t*));
        memset(newRoot + __kmp_global.threads_capacity, 0,
               (newCapacity - __kmp_global.threads_capacity) * sizeof(kmp_root_t*));

        if(!old_tp_cached && __kmp_global.tp_cached && newCapacity > __kmp_global.tp_capacity) {
            /* __kmp_global.tp_cached has changed, i.e. __kmpc_threadprivate_cached has allocated a threadprivate cache
        ...
        __kmp_acquire_bootstrap_lock(&__kmp_global.tp_cached_lock);
        if(!old_tp_cached && __kmp_global.tp_cached && newCapacity > __kmp_global.tp_capacity) {
            /* Same check as above, but this time with the lock so we can be sure if we can succeed. */
            __kmp_release_bootstrap_lock(&__kmp_global.tp_cached_lock);
            __kmp_free(newThreads);
            continue; /* start over and try again */
        } else {
            /* success */
            // __kmp_free( __kmp_global.threads ); // ATT: It leads to crash. Need to be investigated.
            //
            *(kmp_info_t**volatile*)&__kmp_global.threads = newThreads;
            *(kmp_root_t**volatile*)&__kmp_global.root = newRoot;
            added += newCapacity - __kmp_global.threads_capacity;
            *(volatile int*)&__kmp_global.threads_capacity = newCapacity;
            __kmp_release_bootstrap_lock(&__kmp_global.tp_cached_lock);
...
/* we must have the __kmp_global.initz_lock held at this point */
...
__kmp_register_root( int initial_thread )
{
    ...
    int         gtid;
    int         capacity;
   // printf("__kmp_register_root antes lock\n");
    __kmp_acquire_bootstrap_lock( &__kmp_global.forkjoin_lock );
   // printf("__kmp_register_root despues lock\n");
    ...
    /*
        ...
        "__kmp_global.all_nth >= __kmp_global.threads_capacity" condition does not work as expected -- it may
        return false (that means there is at least one empty slot in __kmp_global.threads array), but it
    ...
    */
    capacity = __kmp_global.threads_capacity;
    if ( ! initial_thread && TCR_PTR(__kmp_global.threads[0]) == NULL ) {
        -- capacity;
    }; // if

    /* see if there are too many threads */
    if ( __kmp_global.all_nth >= capacity && !__kmp_expand_threads( 1, 1 ) ) {
        if ( __kmp_global.tp_cached ) {
            __kmp_msg(
                kmp_ms_fatal,
                KMP_MSG( CantRegisterNewThread ),
                KMP_HNT( Set_ALL_THREADPRIVATE, __kmp_global.tp_capacity ),
    ...
    /* Don't reassign the zero slot since we need that to only be used by initial
       thread */
    for( gtid=(initial_thread ? 0 : 1) ; TCR_PTR(__kmp_global.threads[gtid]) != NULL ; gtid++ )
        ;
    KA_TRACE( 1, ("__kmp_register_root: found slot in threads array: T#%d\n", gtid ));
    KMP_ASSERT( gtid < __kmp_global.threads_capacity );

    /* update global accounting */
    __kmp_global.all_nth ++;
    TCW_4(__kmp_global.nth, __kmp_global.nth + 1);

    /* setup this new hierarchy */
    if( ! ( root = __kmp_global.root[gtid] )) {
        root = __kmp_global.root[gtid] = (kmp_root_t*) __kmp_allocate( sizeof(kmp_root_t) );
    ...
    TCW_SYNC_PTR(__kmp_global.threads[gtid], root_thread);
    ...
    KA_TRACE( 20, ("__kmp_register_root: T#%d init T#%d(%d:%d) arrived\n",
                    gtid, __kmp_gtid_from_tid( 0, root->r.r_hot_team ),
                    root->r.r_hot_team->t.t_id, 0 ) );
...
/* Resets a root thread and clear its root and hot teams.
   Returns the number of __kmp_global.threads entries directly and indirectly freed.
*/
static int
__kmp_reset_root(int gtid, kmp_root_t *root)
{
...
#if KMP_NESTED_HOT_TEAMS
    if( __kmp_global.hot_teams_max_level > 1 ) {  // need to free nested hot teams and their threads if any
        for( i = 0; i < hot_team->t.t_nproc; ++i ) {
            kmp_info_t *th = hot_team->t.t_threads[i];
            n += __kmp_free_hot_teams( root, th, 1, __kmp_global.hot_teams_max_level );
    ...
    TCW_4(__kmp_global.nth, __kmp_global.nth - 1); // __kmp_reap_thread will decrement __kmp_global.all_nth.
    __kmp_reap_thread( root->r.r_uber_thread, 1 );

        // We canot put root thread to __kmp_global.thread_pool, so we have to reap it istead of freeing.
...
{
    //printf("__kmp_unregister_root_current_thread gtid=%d\n",gtid);
    ...
    //printf("__kmp_unregister_root_current_thread 2 gtid=%d\n",gtid);

    __kmp_acquire_bootstrap_lock( &__kmp_global.forkjoin_lock );
    if( TCR_4(__kmp_global.g.g_done) || !__kmp_global.init_serial ) {
               // printf("__kmp_unregister_root_current_thread 25 gtid=%d\n",gtid);

        KC_TRACE( 10, ("__kmp_unregister_root_current_thread: already finished, exiting T#%d\n", gtid ));
        __kmp_release_bootstrap_lock( &__kmp_global.forkjoin_lock );
        return;
    }
        //printf("__kmp_unregister_root_current_thread 3 gtid=%d\n",gtid);

    kmp_root_t *root = __kmp_global.root[gtid];

    KMP_DEBUG_ASSERT( __kmp_global.threads && __kmp_global.threads[gtid] );
    KMP_ASSERT( KMP_UBER_GTID( gtid ));
    KMP_ASSERT( root == __kmp_global.threads[gtid]->th.th_root );
    KMP_ASSERT( root->r.r_active == FALSE );


    KMP_MB();
        //printf("__kmp_unregister_root_current_thread 4 gtid=%d\n",gtid);

#if OMP_41_ENABLED
   kmp_info_t * thread = __kmp_global.threads[gtid];
   kmp_team_t * team = thread->th.th_team;
   kmp_task_team_t *   task_team = thread->th.th_task_team;
        //printf("__kmp_unregister_root_current_thread 5 gtid=%d\n",gtid);

   // we need to wait for the proxy tasks before finishing the thread
   if ( task_team != NULL && task_team->tt.tt_found_proxy_tasks ) {
        __kmp_task_team_wait(thread, team, 1 );
   }
#endif
        //printf("__kmp_unregister_root_current_thread 6 gtid=%d\n",gtid);
    ...
    __kmp_release_bootstrap_lock( &__kmp_global.forkjoin_lock );
        //printf("__kmp_unregister_root_current_thread (end)\n");
...
__kmp_get_global_thread_id( )
{
    ...
    KA_TRACE( 1000, ( "*** __kmp_get_global_thread_id: entering, nproc=%d  all_nproc=%d\n",
                      __kmp_global.nth, __kmp_global.all_nth ));
    ...
    /* JPH - to handle the case where __kmpc_end(0) is called immediately prior to a
             ...
             __kmp_global.init_gtid for this to work.  */

    if ( !TCR_4(__kmp_global.init_gtid) ) return KMP_GTID_DNE;
...
{
    int gtid;
        KA_TRACE( 1000, ( "*** __kmp_get_global_thread_id_reg: before global\n" ));
        //printf("__kmp_get_global_thread_id_reg 1\n");
    if ( !__kmp_init_global ) {
        __kmp_global_initialize();
    }
        //printf("__kmp_get_global_thread_id_reg 2\n");
        KA_TRACE( 1000, ( "*** __kmp_get_global_thread_id_reg: after global\n" ));

    //printf("__kmp_get_global_thread_id_reg 3\n");    
    if ( !__kmp_global.init_serial ) {
        gtid = KMP_GTID_DNE;
    } else {
    ...
    //printf("__kmp_get_global_thread_id_reg 4\n");
    ...
    if( gtid == KMP_GTID_DNE ) {
        ...
        //printf("__kmp_get_global_thread_id_reg 5\n");
        __kmp_acquire_bootstrap_lock( &__kmp_global.initz_lock );
        if( !__kmp_global.init_serial ) {
            __kmp_do_serial_initialize();
            gtid = __kmp_gtid_get_specific();
...
//printf("__kmp_get_global_thread_id_reg 6\n");
        __kmp_release_bootstrap_lock( &__kmp_global.initz_lock );

        /*__kmp_printf( "+++ %d\n", gtid ); */ /* GROO */
    }
//printf("__kmp_get_global_thread_id_reg 7\n");
...
void
__kmp_abort_process(void)
{

    // Later threads may stall here, but that's ok because abort() will kill them.
    __kmp_acquire_bootstrap_lock( &__kmp_global.exit_lock );
    ...
    __kmp_infinite_loop();
    __kmp_release_bootstrap_lock( &__kmp_global.exit_lock );
...
__kmp_warn( char const * format, ... )
{
    ...
    if ( __kmp_global.generate_warnings == kmp_warnings_off ) {
        return;
    }
...
/* called from within initialization routines where __kmp_global.initz_lock is held */
static int
...
{
    __kmp_global.library = arg;

    switch ( __kmp_global.library ) {
    case library_serial :
        {
...
__kmp_parallel_deo( int *gtid_ref, int *cid_ref, ident_t *loc_ref )
{
    ...
    if( __kmp_global.env_consistency_check ) {
        if( __kmp_global.threads[gtid]->th.th_root->r.r_active )
...
)
{
    ...
    if ( ! get__dynamic_2( parent_team, master_tid ) ) {
        ...
        new_nthreads = __kmp_global.avail_proc - __kmp_global.nth + (root->r.r_active ? 1
    ...
    //
    if ( __kmp_global.nth + new_nthreads - ( root->r.r_active ? 1 :
      root->r.r_hot_team->t.t_nproc ) > __kmp_global.max_nth ) {
        int tl_nthreads = __kmp_global.max_nth - __kmp_global.nth + ( root->r.r_active ? 1 :
          root->r.r_hot_team->t.t_nproc );
        if ( tl_nthreads <= 0 ) {
            tl_nthreads = 1;
        }
        ...
        //
        if ( ! get__dynamic_2( parent_team, master_tid )
          && ( ! __kmp_global.reserve_warn ) ) {
            __kmp_global.reserve_warn = 1;
            __kmp_msg(
                kmp_ms_warning,
    ...
    // __kmp_global.threads[0] == NULL.
    //
    capacity = __kmp_global.threads_capacity;
    if ( TCR_PTR(__kmp_global.threads[0]) == NULL ) {
        --capacity;
    }
    if ( __kmp_global.nth + new_nthreads - ( root->r.r_active ? 1 :
      root->r.r_hot_team->t.t_nproc ) > capacity ) {
        ...
        //
        int slotsRequired = __kmp_global.nth + new_nthreads - ( root->r.r_active ? 1 :
          root->r.r_hot_team->t.t_nproc ) - capacity;
                         ...
                         kmp_info_t *master_th, int master_gtid )
{
    ...
    if( hot_teams ) {  // hot teams array is not allocated if KMP_HOT_TEAMS_MAX_LEVEL=0
        ...
        if( level < __kmp_global.hot_teams_max_level ) {
            if( hot_teams[level].hot_team ) {
                // hot team has already been allocated for given level
                // [MTH]KMP_DEBUG_ASSERT(hot_teams[level].hot_team == team);
    ...
    if ( !use_hot_team ) {
        ...
        for ( i=1 ;  i < team->t.t_nproc ; i++ ) {
             //printf("en fork_team_threads con el nuevo gtid %d\n",i);
            ...
            KA_TRACE( 20, ("__kmp_fork_team_threads: T#%d(%d:%d) init arrived T#%d(%d:%d)\n",
...
{
    if ( __kmp_global.inherit_fp_control ) {
...
{
    if ( __kmp_global.inherit_fp_control && team->t.t_fp_control_saved ) {
        //
...
__kmp_serialized_parallel(ident_t *loc, kmp_int32 global_tid)
{
    ...
    if( this_thr->th.th_team != serial_team ) {
        ...
        if ( __kmp_global.nested_nth.used && ( level + 1 < __kmp_global.nested_nth.used ) ) {
            this_thr->th.th_current_task->td_icvs.nproc = __kmp_global.nested_nth.nth[ level + 1 ];
        }

#if OMP_40_ENABLED
        if ( __kmp_global.nested_proc_bind.used && ( level + 1 < __kmp_global.nested_proc_bind.used ) ) {
            this_thr->th.th_current_task->td_icvs.proc_bind
                = __kmp_global.nested_proc_bind.bind_types[ level + 1 ];
        }
#endif /* OMP_40_ENABLED */
    ...
    )
{
    ...
    //[QTH]printf("__kmp_fork_call: enter T#%d\n", gtid );
///    if ( __kmp_global.stkpadding > 0 &&  __kmp_global.root[gtid] != NULL ) {
///        /* Some systems prefer the stack for the root thread(s) to start with */
///        /* some gap from the parent stack to prevent false sharing. */
///        void *dummy = KMP_ALLOCA(__kmp_global.stkpadding);
///        /* These 2 lines below are so this does not get optimized out */
///        if ( __kmp_global.stkpadding > KMP_MAX_STKPADDING )
///            __kmp_global.stkpadding += (short)((kmp_int64)dummy);
///    }
    ...
    /* setup current data */
    master_th     = __kmp_global.threads[ gtid ]; // AC: potentially unsafe, not in sync with shutdown
    ...
    //[QTH]printf("__kmp_fork_call: 1 T#%d\n", gtid );
    ...
    if( *p_hot_teams == NULL && __kmp_global.hot_teams_max_level > 0 ) {
        *p_hot_teams = (kmp_hot_team_ptr_t*)__kmp_allocate(
                sizeof(kmp_hot_team_ptr_t) * __kmp_global.hot_teams_max_level);
    ...
    //[QTH]printf("__kmp_fork_call: 2 T#%d\n", gtid );
...
/* TODO: revert workaround for Intel(R) 64 tracker #96 */
    ...
    //[QTH]printf("__kmp_fork_call: 3 T#%d\n", gtid );
    ...
    //[QTH]printf("__kmp_fork_call: 4 T#%d\n", gtid );
    ...
    //[QTH]printf("__kmp_fork_call: 5 T#%d\n", gtid );

        {
            ...
            if (! parent_team->t.t_invoke( gtid )) {
                ...
                KA_TRACE( 20, ("__kmp_fork_call: T#%d(%d:0) done microtask for MASTER = %p\n",
                        gtid, parent_team->t.t_id, parent_team->t.t_pkfn ) );
...
#endif
    //[QTH]printf("__kmp_fork_call: 6 T#%d\n", gtid );
    ...
    if ( parent_team->t.t_active_level >= master_th->th.th_current_task->td_icvs.max_active_levels ) {
        ...
        if (nthreads > 1) {
            if ( ( !get__nested(master_th) && (root->r.r_in_parallel
                ...
                && !enter_teams
#endif /* OMP_40_ENABLED */
            ) ) || ( __kmp_global.library == library_serial ) ) {
...
/* AC: If we execute teams from parallel region (on host), then teams should be created
   ...
   then teams and their threads should be created regardless of the nesting setting. */
    ...
    //[QTH]printf("__kmp_fork_call: 7 T#%d\n", gtid );
...
// TODO: revert workaround for Intel(R) 64 tracker #96
    ...
    //[QTH]printf("__kmp_fork_call: 8 T#%d\n", gtid );
    ...
    if ((level+1 < __kmp_global.nested_nth.used) && (__kmp_global.nested_nth.nth[level+1] != nthreads_icv)) {
        nthreads_icv = __kmp_global.nested_nth.nth[level+1];
    }
    else {
        nthreads_icv = 0;  // don't update
    }
    //[QTH]printf("__kmp_fork_call: 9 T#%d\n", gtid );
    ...
    if ( master_th->th.th_current_task->td_icvs.proc_bind == proc_bind_false ) {
        ...
        if ((level+1 < __kmp_global.nested_proc_bind.used)
            && (__kmp_global.nested_proc_bind.bind_types[level+1] != master_th->th.th_current_task->td_icvs.proc_bind)) {
            proc_bind_icv = __kmp_global.nested_proc_bind.bind_types[level+1];
        }
    }
    //[QTH]printf("__kmp_fork_call: 91 T#%d\n", gtid );
    ...
    if ((nthreads_icv > 0)
        ...
        //[QTH]printf("__kmp_fork_call: 92 T#%d\n", gtid );
        ...
        /* allocate a new parallel team */
           //[QTH] printf("__kmp_fork_call: 92b T#%d\n", gtid );
    ...
    //[QTH]printf("__kmp_fork_call: 93 T#%d\n", gtid );
    ...
    //[QTH]printf("__kmp_fork_call: 10 T#%d\n", gtid );
...
// TODO: revert workaround for Intel(R) 64 tracker #96
    ...
    KMP_DEBUG_ASSERT( team == __kmp_global.threads[gtid]->th.th_team );
    KMP_MB();
...
#endif /* OMP_40_ENABLED */
    {
     //[QTH]   printf("Voy al internal fork %d\n",gtid);
        __kmp_internal_fork( loc, gtid, team );
     //[QTH]   printf("Vuelvo del internal fork %d\n",gtid);
...

    {
        ...
        // KMP_TIME_DEVELOPER_BLOCK(USER_master_invoke);
            //[QTH]    printf("Voy a ejecutar esto %p aqui %d\n",team->t.t_invoke, gtid);

        if (! team->t.t_invoke( gtid )) {
            KMP_ASSERT2( 0, "cannot invoke microtask for MASTER thread" );
        }
               //[QTH]                 printf("Vuelvo de ejecutar esto %d\n",gtid);
    ...
    KA_TRACE( 20, ("__kmp_fork_call: parallel exit T#%d\n", gtid ));
   //[QTH] printf("__kmp_fork_call: parallel exit T#%d\n", gtid );
...
__kmp_set_num_threads( int new_nth, int gtid )
{
    ...
    KF_TRACE( 10, ("__kmp_set_num_threads: new __kmp_global.nth = %d\n", new_nth ));
    KMP_DEBUG_ASSERT( __kmp_global.init_serial );

    if (new_nth < 1)
        new_nth = 1;
    else if (new_nth > __kmp_global.max_nth)
        new_nth = __kmp_global.max_nth;
    ...
    if ( __kmp_global.init_parallel && ( ! root->r.r_active )
      && ( root->r.r_hot_team->t.t_nproc > new_nth )
#if KMP_NESTED_HOT_TEAMS
      && __kmp_global.hot_teams_max_level && !__kmp_global.hot_teams_mode
#endif
    ) {
        kmp_team_t *hot_team = root->r.r_hot_team;
        int f;
...
//    kmp_team_t *team;
    ...
    if ( kind < kmp_sched_upper_std ) {
        if ( kind == kmp_sched_static && chunk < KMP_DEFAULT_CHUNK ) {
            ...
            thread->th.th_current_task->td_icvs.sched.r_sched_type = __kmp_global.sch_map[ kind - kmp_sched_lower - 1 ];
        }
    } else {
        //    __kmp_global.sch_map[ kind - kmp_sched_lower_ext + kmp_sched_upper_std - kmp_sched_lower - 2 ];
...
// This routine created because pairs (__kmp_global.sched, __kmp_global.chunk) and (__kmp_global.sched_static, __kmp_global.sched_guided)
// may be changed by kmp_set_defaults independently. So one can get the updated schedule here.

    kmp_r_sched_t r_sched;

    // create schedule from 4 globals: __kmp_global.sched, __kmp_global.chunk, __kmp_global.sched_static, __kmp_global.sched_guided
    // __kmp_global.sched should keep original value, so that user can set KMP_SCHEDULE multiple times,
    // and thus have different run-time schedules in different roots (even in OMP 2.5)
    if ( __kmp_global.sched == kmp_sch_static ) {
        r_sched.r_sched_type = __kmp_global.sched_static; // replace STATIC with more detailed schedule (balanced or greedy)
    } else if ( __kmp_global.sched == kmp_sch_guided_chunked ) {
        r_sched.r_sched_type = __kmp_global.sched_guided; // replace GUIDED with more detailed schedule (iterative or analytical)
    } else {
        r_sched.r_sched_type = __kmp_global.sched;  // (STATIC_CHUNKED), or (DYNAMIC_CHUNKED), or other
    }

    if ( __kmp_global.chunk < KMP_DEFAULT_CHUNK ) { // __kmp_global.chunk may be wrong here (if it was not ever set)
        r_sched.chunk = KMP_DEFAULT_CHUNK;
    } else {
        r_sched.chunk = __kmp_global.chunk;
    }
...
__kmp_print_structure(
    ...
    {
        int gtid;
        for ( gtid = 0; gtid < __kmp_global.threads_capacity; ++ gtid ) {
            __kmp_printf( "%2d", gtid );
            if ( __kmp_global.threads != NULL ) {
                __kmp_printf( " %p", __kmp_global.threads[ gtid ] );
            }; // if
            if ( __kmp_global.root != NULL ) {
                __kmp_printf( " %p", __kmp_global.root[ gtid ] );
            }; // if
            __kmp_printf( "\n" );
        }; // for gtid
    }

    // Print out __kmp_global.threads array.
    __kmp_printf( "\n------------------------------\nThreads\n------------------------------\n" );
    if ( __kmp_global.threads != NULL ) {
        int gtid;
        for ( gtid = 0; gtid < __kmp_global.threads_capacity; ++ gtid ) {
            kmp_info_t const * thread = __kmp_global.threads[ gtid ];
            if ( thread != NULL ) {
    ...
    // Print out __kmp_global.root array.
    __kmp_printf( "\n------------------------------\nUbers\n------------------------------\n" );
    if ( __kmp_global.root != NULL ) {
        int gtid;
        for ( gtid = 0; gtid < __kmp_global.threads_capacity; ++ gtid ) {
            kmp_root_t const * root = __kmp_global.root[ gtid ];
            if ( root != NULL ) {
                __kmp_printf( "GTID %2d %p:\n", gtid, root );
    ...
    // Print out __kmp_global.thread_pool and __kmp_global.team_pool.
    __kmp_printf( "\n------------------------------\nPools\n------------------------------\n" );
    __kmp_print_structure_thread(   "Thread pool:          ", (kmp_info_t *)__kmp_global.thread_pool );
    __kmp_print_structure_team(     "Team pool:            ", (kmp_team_t *)__kmp_global.team_pool );
    __kmp_printf( "\n" );
...
__kmp_allocate_thread( kmp_root_t *root, kmp_team_t *team, int new_tid )
{
    ...
    if ( __kmp_global.thread_pool ) {

        new_thr = (kmp_info_t*)__kmp_global.thread_pool;
        __kmp_global.thread_pool = (volatile kmp_info_t *) new_thr->th.th_next_pool;
        ...
        KMP_DEBUG_ASSERT( __kmp_global.nth < __kmp_global.threads_capacity );
        KMP_DEBUG_ASSERT( __kmp_global.thread_pool_nth >= 0 );
    ...
    //printf("%d == %d??\n",__kmp_global.nth, __kmp_global.all_nth);
    KMP_ASSERT( __kmp_global.nth    == __kmp_global.all_nth );
    KMP_ASSERT( __kmp_global.all_nth < __kmp_global.threads_capacity );

    KMP_MB();
    for( new_gtid=1 ; TCR_PTR(__kmp_global.threads[new_gtid]) != NULL; ++new_gtid ) {
        KMP_DEBUG_ASSERT( new_gtid < __kmp_global.threads_capacity );
    }

    /* allocate space for it. */
    new_thr = (kmp_info_t*) __kmp_allocate( sizeof(kmp_info_t) );

    TCW_SYNC_PTR(__kmp_global.threads[new_gtid], new_thr);
...
///#endif
    ...
    new_thr->th.th_info.ds.ds_thread = GLT_UNIT_NULL;//ABT_THREAD_NULL;
    new_thr->th.th_info.ds.ds_tasklet = GLT_UNIT_NULL;//ABT_TASK_NULL;
...
///    __kmp_create_worker( new_gtid, new_thr, __kmp_global.stksize );
...
__kmp_reinitialize_team( kmp_team_t *team, kmp_internal_control_t *new_icvs, ident_t *loc ) {
    ...
    KMP_DEBUG_ASSERT( ( ! TCR_4(__kmp_global.init_parallel) ) || new_icvs->nproc );
    team->t.t_ident = loc;
    ...
    int argc USE_NESTED_HOT_ARG(kmp_info_t *master) )
{
    ...
    if( master ) {
        ...
        if( level < __kmp_global.hot_teams_max_level && hot_teams && hot_teams[level].hot_team )
        {   // hot team has already been allocated for given level
            use_hot_team = 1;
        } else {
            use_hot_team = 0;
...
///# endif /* KMP_AFFINITY_SUPPORTED */
            ...
            team->t.t_size_changed = 1;
#if KMP_NESTED_HOT_TEAMS
            if( __kmp_global.hot_teams_mode == 0 ) {
...
///#endif
            ...
            if( hot_teams[level].hot_team_nth >= new_nproc ) {
                ...
                KMP_DEBUG_ASSERT(__kmp_global.hot_teams_mode == 1);
...
///# endif
        ...
        // Needed just in case it is not created before. 
        // In massiveThreads if it is not created, it cannot be released.
    ...
    KMP_MB();
    for( team = (kmp_team_t*) __kmp_global.team_pool ; (team) ; )
    {
...
__kmp_free_team( kmp_root_t *root, kmp_team_t *team  USE_NESTED_HOT_ARG(kmp_info_t *master) )
{
    ...
    if( ! use_hot_team ) {
        ...
        for ( f = 1; f < team->t.t_nproc; ++ f ) {
            //[MTH]KMP_DEBUG_ASSERT( team->t.t_threads[ f ] );
            __kmp_free_thread( team->t.t_threads[ f ] );
            team->t.t_threads[ f ] = NULL;
        }
        ...
        team->t.t_next_pool  = (kmp_team_t*) __kmp_global.team_pool;
        __kmp_global.team_pool        = (volatile kmp_team_t*) team;
    }
...
//
// Now, we always keep the free list (__kmp_global.thread_pool) sorted by gtid.
...
__kmp_free_thread( kmp_info_t *this_th )
{
    ...
    // __kmp_global.thread_pool itself.
    //
    ...
    if ( __kmp_thread_pool_insert_pt != NULL ) {
        ...
        scan = (kmp_info_t **)&__kmp_global.thread_pool;
    }
...
__kmp_reap_thread(
    ...
    // It is assumed __kmp_global.forkjoin_lock is acquired.
    ...
    if ( ! is_root ) {
        ...
        // __kmp_global.thread_pool_active_nth yet, then the global counter
        ...
        if ( thread->th.th_active_in_pool ) {
            ...
            KMP_TEST_THEN_DEC32(
              (kmp_int32 *) &__kmp_global.thread_pool_active_nth );
            KMP_DEBUG_ASSERT( TCR_4(__kmp_global.thread_pool_active_nth) >= 0 );
        }
    ...
    KMP_DEBUG_ASSERT( __kmp_global.threads[ gtid ] == thread );
    TCW_SYNC_PTR(__kmp_global.threads[gtid], NULL);

    -- __kmp_global.all_nth;
    // __kmp_global.nth was decremented when thread is added to the pool.
...
__kmp_register_library_startup(
    ...
    //printf("__kmp_register_library_startup\n");
        ...
        if ( value != NULL && strcmp( value, __kmp_registration_str ) == 0 ) {
            ...
            switch ( neighbor ) {
                ...
                case 1 : {    // Neighbor is alive.
                    ...
                    __kmp_global.duplicate_library_ok = 1;
...
__kmp_invoke_task_func( int gtid )
{
    ...
    kmp_info_t  *this_thr = __kmp_global.threads[ gtid ];
    kmp_team_t  *team     = this_thr->th.th_team;
    //[QTH]printf("T%d en __kmp_invoke_task_func\n", gtid);
    __kmp_run_before_invoked_task( gtid, tid, this_thr, team );
        //[QTH]printf("T%d en __kmp_invoke_task_func 2\n", gtid);
    ...
    {
        KMP_TIME_BLOCK(OMP_work);
                //[QTH]printf("T%d en __kmp_invoke_task_func %p 3 with  %d args\n", gtid,team->t.t_pkfn,(int) team->t.t_argc);

        rc = __kmp_invoke_microtask( (microtask_t) TCR_SYNC_PTR(team->t.t_pkfn),
                                    gtid, tid, (int) team->t.t_argc, (void **) team->t.t_argv
                                     );
    }
       //[QTH] printf("T%d en __kmp_invoke_task_func 4\n", gtid);

    __kmp_run_after_invoked_task( gtid, tid, this_thr, team );
       //[QTH] printf("T%d en __kmp_invoke_task_func 5\n", gtid);
...
__kmp_teams_master( int gtid )
{
    ...
    kmp_info_t *thr = __kmp_global.threads[ gtid ];
...
__kmp_invoke_teams_master( int gtid )
{
    ...
    kmp_team_t  *team     = this_thr->th.th_team;
    #if KMP_DEBUG
    if ( !__kmp_global.threads[gtid]-> th.th_team->t.t_serialized )
        KMP_DEBUG_ASSERT( (void*)__kmp_global.threads[gtid]-> th.th_team->t.t_pkfn == (void*)__kmp_teams_master );
    #endif
...
__kmp_push_num_teams( ident_t *id, int gtid, int num_teams, int num_threads )
{
    ...
    if( num_teams > __kmp_global.max_nth ) { // if too many teams requested?
        if ( !__kmp_global.reserve_warn ) {
            __kmp_global.reserve_warn = 1;
            __kmp_msg(
                kmp_ms_warning,
                KMP_MSG( CantFormThrTeam, num_teams, __kmp_global.max_nth ),
                KMP_HNT( Unset_ALL_THREADS ),
                __kmp_msg_null
            );
        }
        num_teams = __kmp_global.max_nth;
    }
    ...
    if( num_threads == 0 ) {
        if( !TCR_4(__kmp_global.init_middle) )
            __kmp_middle_initialize();  // get __kmp_global.avail_proc calculated
        num_threads = __kmp_global.avail_proc / num_teams;
        if( num_teams * num_threads > __kmp_global.max_nth ) {
            // adjust num_threads w/o warning as it is not user setting
            num_threads = __kmp_global.max_nth / num_teams;
        }
    } else {
        if( num_teams * num_threads > __kmp_global.max_nth ) {
            int new_threads = __kmp_global.max_nth / num_teams;
            if ( !__kmp_global.reserve_warn ) { // user asked for too many threads
                __kmp_global.reserve_warn = 1;  // that conflicts with OMP_THREAD_LIMIT
                __kmp_msg(
                    kmp_ms_warning,
...
__kmp_internal_fork( ident_t *id, int gtid, kmp_team_t *team )
{
    ...
    //[MTH]KMP_ASSERT(       KMP_MASTER_GTID(gtid) );
    KMP_MB();       /* Flush all pending memory write invalidates.  */
...
#ifdef KMP_DEBUG
  /*[QTH]  for( f=0 ; f<team->t.t_nproc ; f++ ) {
        KMP_DEBUG_ASSERT( team->t.t_threads[f] &&
                          team->t.t_threads[f]->th.th_team_nproc == team->t.t_nproc );
    }*/
#endif /* KMP_DEBUG */
...
#endif
            if (th->th.th_info.ds.ds_thread == GLT_UNIT_NULL) {
...
__kmp_internal_join( ident_t *id, int gtid, kmp_team_t *team )
{
    ...
    KMP_DEBUG_ASSERT( team );
    KMP_DEBUG_ASSERT( this_thr->th.th_team  ==  team );
    //[QTH]KMP_ASSERT(       KMP_MASTER_GTID(gtid) );
...
#ifdef KMP_DEBUG
    if (__kmp_global.threads[gtid] && __kmp_global.threads[gtid]->th.th_team_nproc != team->t.t_nproc ) {
        __kmp_printf("GTID: %d, __kmp_global.threads[%d]=%p\n",gtid, gtid, __kmp_global.threads[gtid]);
        __kmp_printf("__kmp_global.threads[%d]->th.th_team_nproc=%d, TEAM: %p, team->t.t_nproc=%d\n",
                     gtid, __kmp_global.threads[gtid]->th.th_team_nproc, team, team->t.t_nproc);
        __kmp_print_structure();
    }
    KMP_DEBUG_ASSERT( __kmp_global.threads[gtid] &&
    ...
    __kmp_wait_child_tasks(this_thr, FALSE);
        KA_TRACE( 20, ("__kmp_internal_join: after __kmp_wait_child_tasks: T#%d waited T#%d\n",
    ...
    KMP_MB();       /* Flush all pending memory write invalidates.  */
    //[MTH]KMP_ASSERT( this_thr->th.th_team  ==  team );
}
...
__kmp_load_balance_nproc( kmp_root_t *root, int set_nproc )
{
    ...
    //
    pool_active = TCR_4(__kmp_global.thread_pool_active_nth);
    ...
    //
    system_active = __kmp_get_load_balance( __kmp_global.avail_proc + team_curr_active );
    ...
    if ( system_active < 0 ) {
        ...
        //
        retval = __kmp_global.avail_proc - __kmp_global.nth + (root->r.r_active ? 1
          : root->r.r_hot_team->t.t_nproc);
        if ( retval > set_nproc ) {
            retval = set_nproc;
        }
    ...
    retval = __kmp_global.avail_proc - system_active + team_curr_active;
    if ( retval > set_nproc ) {
        retval = set_nproc;
    }
    if ( retval < KMP_MIN_NTH ) {
...
__kmp_user_set_library (enum library_type arg)
{
    ...
    case library_turnaround :
        thread->th.th_set_nproc = 0;
        set__nproc( thread, __kmp_global.dflt_team_nth ? __kmp_global.dflt_team_nth : __kmp_global.dflt_team_nth_ub );
        break;
    case library_throughput :
        thread->th.th_set_nproc = 0;
        set__nproc( thread, __kmp_global.dflt_team_nth ? __kmp_global.dflt_team_nth : __kmp_global.dflt_team_nth_ub );
...
__kmp_aux_set_stacksize( size_t arg )
{
    ...
    if (! TCR_4(__kmp_global.init_parallel)) {
        ...
        if (value < __kmp_global.sys_min_stksize )
            value = __kmp_global.sys_min_stksize ;
        else if (value > KMP_MAX_STKSIZE)
            value = KMP_MAX_STKSIZE;

        __kmp_global.stksize = value;

        __kmp_global.env_stksize = TRUE;    /* was KMP_STACKSIZE specified? */
    }
...
__kmp_aux_set_defaults(
    ...
    if (__kmp_global.settings
#if OMP_40_ENABLED
        || __kmp_global.display_env || __kmp_global.display_env_verbose
#endif // OMP_40_ENABLED
        ) {
        __kmp_env_print();
    }
...
#if KMP_ARCH_X86_64 && (KMP_OS_LINUX || KMP_OS_WINDOWS)
                if( __kmp_global.mic_type != non_mic ) {
                    teamsize_cutoff = 8;
                }
#endif
///                if( tree_available ) {
...
///                    }
    ...
    if( __kmp_global.force_reduction_method != reduction_method_not_defined && team_size != 1) {

        PACKED_REDUCTION_METHOD_T forced_retval;

        int atomic_available;
///        int tree_available;

        switch( ( forced_retval = __kmp_global.force_reduction_method ) )
        {
