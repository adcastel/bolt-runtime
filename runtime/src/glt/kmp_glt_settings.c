/*
 * kmp_abt_settings.c -- Initialize environment variables
 */
...
__kmp_initial_threads_capacity( int req_nproc )
{
    ...
    /* MIN( MAX( 32, 4 * $OMP_NUM_THREADS, 4 * omp_get_num_procs() ), __kmp_global.max_nth) */
    if (nth < (4 * req_nproc))
        nth = (4 * req_nproc);
    if (nth < (4 * __kmp_global.xproc))
        nth = (4 * __kmp_global.xproc);

    if (nth > __kmp_global.max_nth)
        nth = __kmp_global.max_nth;
...
__kmp_default_tp_capacity( int req_nproc, int max_nth, int all_threads_specified) {
    ...
    if(all_threads_specified)
        return max_nth;
    /* MIN( MAX (128, 4 * $OMP_NUM_THREADS, 4 * omp_get_num_procs() ), __kmp_global.max_nth ) */
    if (nth < (4 * req_nproc))
        nth = (4 * req_nproc);
    if (nth < (4 * __kmp_global.xproc))
        nth = (4 * __kmp_global.xproc);
...
__kmp_stg_parse_all_threads( char const * name, char const * value, void * data ) {
    ...
    if ( ! __kmp_strcasecmp_with_sentinel( "all", value, 0 ) ) {
        __kmp_global.max_nth = __kmp_global.xproc;
        __kmp_global.allThreadsSpecified = 1;
    } else {
        __kmp_stg_parse_int( name, value, 1, __kmp_global.sys_max_nth, & __kmp_global.max_nth );
        __kmp_global.allThreadsSpecified = 0;
    }
    K_DIAG( 1, ( "__kmp_global.max_nth == %d\n", __kmp_global.max_nth ) );

} // __kmp_stg_parse_all_threads

static void
__kmp_stg_print_all_threads( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_int( buffer, name, __kmp_global.max_nth );
...
__kmp_stg_parse_duplicate_lib_ok( char const * name, char const * value, void * data ) {
    ...
    __kmp_stg_parse_bool( name, value, & __kmp_global.duplicate_library_ok );
} // __kmp_stg_parse_duplicate_lib_ok

static void
__kmp_stg_print_duplicate_lib_ok( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_bool( buffer, name, __kmp_global.duplicate_library_ok );
...
__kmp_stg_parse_inherit_fp_control( char const * name, char const * value, void * data ) {
    __kmp_stg_parse_bool( name, value, & __kmp_global.inherit_fp_control );
} // __kmp_stg_parse_inherit_fp_control

static void
__kmp_stg_print_inherit_fp_control( kmp_str_buf_t * buffer, char const * name, void * data ) {
#if KMP_DEBUG
    __kmp_stg_print_bool( buffer, name, __kmp_global.inherit_fp_control );
#endif /* KMP_DEBUG */
...
__kmp_stg_parse_wait_policy( char const * name, char const * value, void * data ) {
    ...
    if ( wait->omp ) {
        if ( __kmp_str_match( "ACTIVE", 1, value ) ) {
           __kmp_global.library = library_turnaround;
    ...
    __kmp_aux_set_library( __kmp_global.library );
...
__kmp_stg_parse_settings( char const * name, char const * value, void * data ) {
    __kmp_stg_parse_bool( name, value, & __kmp_global.settings );
} // __kmp_stg_parse_settings

static void
__kmp_stg_print_settings( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_bool( buffer, name, __kmp_global.settings );
...
__kmp_stg_parse_stackoffset( char const * name, char const * value, void * data ) {
    __kmp_stg_parse_size(
        ...
        NULL,                             //
        & __kmp_global.stkoffset,                // Var to initialize
        1
    );
...
__kmp_stg_print_stackoffset( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_size( buffer, name, __kmp_global.stkoffset );
...
__kmp_stg_parse_stacksize( char const * name, char const * value, void * data ) {
    ...
    __kmp_stg_parse_size(
        name,                     // Env var name
        value,                    // Env var value
        __kmp_global.sys_min_stksize,    // Min value
        KMP_MAX_STKSIZE,          // Max value
        & __kmp_global.env_stksize,      //
        & __kmp_global.stksize,          // Var to initialize
        stacksize->factor
    );
...
__kmp_stg_print_stacksize( kmp_str_buf_t * buffer, char const * name, void * data ) {
    ...
    if( __kmp_env_format ) {
        KMP_STR_BUF_PRINT_NAME_EX(name);
        __kmp_str_buf_print_size( buffer, (__kmp_global.stksize % 1024) ? __kmp_global.stksize / stacksize->factor : __kmp_global.stksize );
        __kmp_str_buf_print( buffer, "'\n" );
    } else {
        __kmp_str_buf_print( buffer, "   %s=", name );
        __kmp_str_buf_print_size( buffer, (__kmp_global.stksize % 1024) ? __kmp_global.stksize / stacksize->factor : __kmp_global.stksize );
...
__kmp_stg_parse_version( char const * name, char const * value, void * data ) {
    __kmp_stg_parse_bool( name, value, & __kmp_global.version );
} // __kmp_stg_parse_version

static void
__kmp_stg_print_version( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_bool( buffer, name, __kmp_global.version );
...
__kmp_stg_parse_warnings( char const * name, char const * value, void * data ) {
    __kmp_stg_parse_bool( name, value, & __kmp_global.generate_warnings );
    if (__kmp_global.generate_warnings != kmp_warnings_off) {   // AC: we have only 0/1 values documented,
        __kmp_global.generate_warnings = kmp_warnings_explicit; //     so reset it to explicit in order to
...
__kmp_stg_print_warnings( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_bool( buffer, name, __kmp_global.generate_warnings ); // AC: TODO: change to print_int?
...
__kmp_stg_parse_nested( char const * name, char const * value, void * data ) {
    __kmp_stg_parse_bool( name, value, & __kmp_global.dflt_nested );
} // __kmp_stg_parse_nested

static void
__kmp_stg_print_nested( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_bool( buffer, name, __kmp_global.dflt_nested );
    ...
    for ( ; ; ) {
        ...
        if ( *scan >= '0' && *scan <= '9' ) {
            ...
            if ( num < KMP_MIN_NTH ) {
                msg = KMP_I18N_STR( ValueTooSmall );
                num = KMP_MIN_NTH;
            } else if ( num > __kmp_global.sys_max_nth ) {
                msg = KMP_I18N_STR( ValueTooLarge );
                num = __kmp_global.sys_max_nth;
            }
            if ( msg != NULL ) {
                // Message is not empty. Print warning.
...
__kmp_stg_parse_num_threads( char const * name, char const * value, void * data ) {
    ...
    if ( ! __kmp_strcasecmp_with_sentinel( "all", value, 0 ) ) {
        ...
        __kmp_global.nested_nth.nth = ( int* )KMP_INTERNAL_MALLOC( sizeof( int ) );
        __kmp_global.nested_nth.size = __kmp_global.nested_nth.used = 1;
        __kmp_global.nested_nth.nth[0] = __kmp_global.dflt_team_nth = __kmp_global.dflt_team_nth_ub = __kmp_global.xproc;
    } else {
        __kmp_parse_nested_num_threads( name, value, & __kmp_global.nested_nth );
        if ( __kmp_global.nested_nth.nth ) {
            __kmp_global.dflt_team_nth = __kmp_global.nested_nth.nth[0];
            if ( __kmp_global.dflt_team_nth_ub < __kmp_global.dflt_team_nth ) {
                __kmp_global.dflt_team_nth_ub = __kmp_global.dflt_team_nth;
            }
        }
    }; // if
    K_DIAG( 1, ( "__kmp_global.dflt_team_nth == %d\n", __kmp_global.dflt_team_nth ) );
...
__kmp_stg_print_num_threads( kmp_str_buf_t * buffer, char const * name, void * data ) {
    ...
    if ( __kmp_global.nested_nth.used ) {
        kmp_str_buf_t buf;
        __kmp_str_buf_init( &buf );
        for ( int i = 0; i < __kmp_global.nested_nth.used; i++) {
            __kmp_str_buf_print( &buf, "%d", __kmp_global.nested_nth.nth[i] );
            if ( i < __kmp_global.nested_nth.used - 1 ) {
...
__kmp_stg_parse_max_task_priority(char const *name, char const *value, void *data) {
    __kmp_stg_parse_int(name, value, 0, KMP_MAX_TASK_PRIORITY_LIMIT, &__kmp_global.max_task_priority);
} // __kmp_stg_parse_max_task_priority

static void
__kmp_stg_print_max_task_priority(kmp_str_buf_t *buffer, char const *name, void *data) {
    __kmp_stg_print_int(buffer, name, __kmp_global.max_task_priority);
...
__kmp_stg_parse_hot_teams_level( char const * name, char const * value, void * data ) {
    ...
    __kmp_stg_parse_int( name, value, 0, KMP_MAX_ACTIVE_LEVELS_LIMIT, & __kmp_global.hot_teams_max_level );
} // __kmp_stg_parse_hot_teams_level

static void
__kmp_stg_print_hot_teams_level( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_int( buffer, name, __kmp_global.hot_teams_max_level );
...
__kmp_stg_parse_hot_teams_mode( char const * name, char const * value, void * data ) {
    ...
    __kmp_stg_parse_int( name, value, 0, KMP_MAX_ACTIVE_LEVELS_LIMIT, & __kmp_global.hot_teams_mode );
} // __kmp_stg_parse_hot_teams_mode

static void
__kmp_stg_print_hot_teams_mode( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_int( buffer, name, __kmp_global.hot_teams_mode );
...
///	__kmp_stg_parse_bool( name, value, & __kmp_global.handle_signals );
///} // __kmp_stg_parse_handle_signals
///
///static void
///__kmp_stg_print_handle_signals( kmp_str_buf_t * buffer, char const * name, void * data ) {
///    __kmp_stg_print_bool( buffer, name, __kmp_global.handle_signals );
...
__kmp_stg_print_align_alloc( kmp_str_buf_t * buffer, char const * name, void * data ) {
        __kmp_stg_print_size( buffer, name, __kmp_global.align_alloc );
...
__kmp_stg_parse_abort_delay( char const * name, char const * value, void * data ) {
    // Units of KMP_DELAY_ABORT are seconds, units of __kmp_global.abort_delay is milliseconds.
    int delay = __kmp_global.abort_delay / 1000;
    __kmp_stg_parse_int( name, value, 0, INT_MAX / 1000, & delay );
    __kmp_global.abort_delay = delay * 1000;
} // __kmp_stg_parse_abort_delay

static void
__kmp_stg_print_abort_delay( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_int( buffer, name, __kmp_global.abort_delay );
    ...
    if ( reduction->force ) {
        if( value != 0 ) {
            if( __kmp_str_match( "critical", 0, value ) )
               __kmp_global.force_reduction_method = critical_reduce_block;
            else if( __kmp_str_match( "atomic", 0, value ) )
               __kmp_global.force_reduction_method = atomic_reduce_block;
            else if( __kmp_str_match( "tree", 0, value ) )
               __kmp_global.force_reduction_method = tree_reduce_block;
            else {
                KMP_FATAL( UnknownForceReduction, name, value );
            }
        }
    } else {
        __kmp_stg_parse_bool( name, value, & __kmp_global.determ_red );
        if( __kmp_global.determ_red ) {
            __kmp_global.force_reduction_method = tree_reduce_block;
        } else {
            __kmp_global.force_reduction_method = reduction_method_not_defined;
        }
    }
    K_DIAG( 1, ( "__kmp_global.force_reduction_method == %d\n", __kmp_global.force_reduction_method ) );
...
__kmp_stg_print_force_reduction( kmp_str_buf_t * buffer, char const * name, void * data ) {
    ...
    kmp_stg_fr_data_t * reduction = (kmp_stg_fr_data_t *) data;
    if ( reduction->force ) {
        if( __kmp_global.force_reduction_method == critical_reduce_block) {
            __kmp_stg_print_str( buffer, name, "critical");
        } else if ( __kmp_global.force_reduction_method == atomic_reduce_block ) {
            __kmp_stg_print_str( buffer, name, "atomic");
        } else if ( __kmp_global.force_reduction_method == tree_reduce_block ) {
        ...
        __kmp_stg_print_bool( buffer, name, __kmp_global.determ_red );
    }
...
__kmp_stg_parse_all_threadprivate( char const * name, char const * value, void * data ) {
    __kmp_stg_parse_int( name, value, __kmp_global.allThreadsSpecified ? __kmp_global.max_nth : 1, __kmp_global.max_nth,
        & __kmp_global.tp_capacity );
} // __kmp_stg_parse_all_threadprivate

static void
__kmp_stg_print_all_threadprivate( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_int( buffer, name, __kmp_global.tp_capacity );
...
__kmp_stg_parse_foreign_threads_threadprivate( char const * name, char const * value, void * data ) {
    __kmp_stg_parse_bool( name, value, & __kmp_global.foreign_tp );
...
__kmp_stg_print_foreign_threads_threadprivate( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_bool( buffer, name, __kmp_global.foreign_tp );
...
///
///# if OMP_40_ENABLED
///    KMP_DEBUG_ASSERT( ( __kmp_global.nested_proc_bind.bind_types != NULL )
///      && ( __kmp_global.nested_proc_bind.used > 0 ) );
///# endif
///
///    while ( *buf != '\0' ) {
///        start = next = buf;
...
///# if OMP_40_ENABLED
///            __kmp_global.nested_proc_bind.bind_types[0] = proc_bind_false;
///# endif
///            buf = next;
///        } else if (__kmp_match_str("scatter", buf, (const char **)&next)) {
///            set_type( affinity_scatter );
///# if OMP_40_ENABLED
///            __kmp_global.nested_proc_bind.bind_types[0] = proc_bind_intel;
///# endif
///            buf = next;
...
///    //
///    if ( __kmp_global.nested_proc_bind.bind_types[0] == proc_bind_default ) {
///        __kmp_global.nested_proc_bind.bind_types[0] = proc_bind_true;
///    }
///
...
///        __kmp_str_buf_print( buffer, "   %s", name );
///    }
///    if ( ( __kmp_global.nested_proc_bind.used == 0 )
///      || ( __kmp_global.nested_proc_bind.bind_types == NULL )
///      || ( __kmp_global.nested_proc_bind.bind_types[0] == proc_bind_false ) ) {
///        __kmp_str_buf_print( buffer, ": %s\n", KMP_I18N_STR( NotDefined ) );
///    }
///    else if ( __kmp_affinity_type == affinity_explicit ) {
...
///# endif /* KMP_AFFINITY_SUPPORTED */
        __kmp_global.nested_proc_bind.used = 1;
        __kmp_global.nested_proc_bind.bind_types[0] = proc_bind_false;
    }
...
///# endif /* KMP_AFFINITY_SUPPORTED */
      ...
      || __kmp_match_str( "true", buf, &next ) ) {
        ...
        //
        if ( __kmp_global.nested_proc_bind.size < nelem ) {
            __kmp_global.nested_proc_bind.bind_types = (kmp_proc_bind_t *)
                KMP_INTERNAL_REALLOC( __kmp_global.nested_proc_bind.bind_types,
                sizeof(kmp_proc_bind_t) * nelem );
            if ( __kmp_global.nested_proc_bind.bind_types == NULL ) {
                KMP_FATAL( MemoryAllocFailed );
            }
            __kmp_global.nested_proc_bind.size = nelem;
        }
        __kmp_global.nested_proc_bind.used = nelem;
        ...
        for (;;) {
            ...
            __kmp_global.nested_proc_bind.bind_types[i++] = bind;
            if ( i >= nelem ) {
                break;
            }
...
__kmp_stg_print_proc_bind( kmp_str_buf_t * buffer, char const * name,
  void * data )
{
    int nelem = __kmp_global.nested_proc_bind.used;
    if( __kmp_env_format ) {
        KMP_STR_BUF_PRINT_NAME;
    } else {
        __kmp_str_buf_print( buffer, "   %s", name );
    ...
    if ( nelem == 0 ) {
        ...
        for ( i = 0; i < nelem; i++ ) {
            switch ( __kmp_global.nested_proc_bind.bind_types[i] ) {
...
__kmp_stg_parse_ld_balance_interval( char const * name, char const * value, void * data )
{
    ...
    if ( interval >= 0 ) {
        __kmp_global.load_balance_interval = interval;
    } else {
        KMP_WARNING( StgInvalidValue, name, value );
    }; // if
...
#if KMP_DEBUG
    __kmp_str_buf_print( buffer, "   %s=%8.6f\n", name, __kmp_global.load_balance_interval );
#endif /* KMP_DEBUG */
...
__kmp_stg_parse_init_at_fork( char const * name, char const * value, void * data ) {
    __kmp_stg_parse_bool( name, value, & __kmp_global.need_register_atfork );
...
__kmp_stg_print_init_at_fork( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_bool( buffer, name, __kmp_global.need_register_atfork_specified );
                ...
                if( *value && semicolon != value ) {
                    ...
                    if ( !__kmp_strcasecmp_with_sentinel( "static", value, sentinel ) ) {
                        if( !__kmp_strcasecmp_with_sentinel( "greedy", comma, ';' ) ) {
                            __kmp_global.sched_static = kmp_sch_static_greedy;
                            continue;
                        ...
                        if ( !__kmp_strcasecmp_with_sentinel( "iterative", comma, ';' ) ) {
                            ...
                            __kmp_global.sched_guided = kmp_sch_guided_analytical_chunked;
                            continue;
                        }
                    }
...
__kmp_stg_print_schedule( kmp_str_buf_t * buffer, char const * name, void * data ) {
    ...
    if ( __kmp_global.sched_static == kmp_sch_static_greedy ) {
        __kmp_str_buf_print( buffer, "%s", "static,greedy");
    } else if ( __kmp_global.sched_static == kmp_sch_static_balanced ) {
        __kmp_str_buf_print ( buffer, "%s", "static,balanced");
    }
    if ( __kmp_global.sched_guided == kmp_sch_guided_iterative_chunked ) {
        __kmp_str_buf_print( buffer, ";%s'\n", "guided,iterative");
    } else if ( __kmp_global.sched_guided == kmp_sch_guided_analytical_chunked ) {
        __kmp_str_buf_print( buffer, ";%s'\n", "guided,analytical");
    }
    ...
    if( value ) {
        ...
        if( length ) {
            ...
            if (!__kmp_strcasecmp_with_sentinel("dynamic", value, ','))          /* DYNAMIC */
                __kmp_global.sched = kmp_sch_dynamic_chunked;
            else if (!__kmp_strcasecmp_with_sentinel("guided", value, ','))      /* GUIDED */
                __kmp_global.sched = kmp_sch_guided_chunked;
// AC: TODO: add AUTO schedule, and pprobably remove TRAPEZOIDAL (OMP 3.0 does not allow it)
                ...
                __kmp_global.sched = kmp_sch_auto;
                if( comma ) {
            ...
            if( value && comma ) {
                __kmp_global.env_chunk = TRUE;

                if(__kmp_global.sched == kmp_sch_static)
                    __kmp_global.sched = kmp_sch_static_chunked;
                ++comma;
                __kmp_global.chunk = __kmp_str_to_int( comma, 0 );
                if ( __kmp_global.chunk < 1 ) {
                    __kmp_global.chunk = KMP_DEFAULT_CHUNK;
                    __kmp_msg( kmp_ms_warning, KMP_MSG( InvalidChunk, name, comma ), __kmp_msg_null );
                    KMP_INFORM( Using_int_Value, name, __kmp_global.chunk );
...
//                } else if ( __kmp_global.chunk < KMP_MIN_CHUNK ) {
//                    __kmp_global.chunk = KMP_MIN_CHUNK;
                } else if ( __kmp_global.chunk > KMP_MAX_CHUNK ) {
                    __kmp_global.chunk = KMP_MAX_CHUNK;
                    __kmp_msg( kmp_ms_warning, KMP_MSG( LargeChunk, name, comma ), __kmp_msg_null );
                    KMP_INFORM( Using_int_Value, name, __kmp_global.chunk );
                }
            } else
                __kmp_global.env_chunk = FALSE;
        } else
            KMP_WARNING( EmptyString, name );
    }
    K_DIAG(1, ("__kmp_global.sched_static == %d\n", __kmp_global.sched_static))
    K_DIAG(1, ("__kmp_global.sched_guided == %d\n", __kmp_global.sched_guided))
    K_DIAG(1, ("__kmp_global.sched == %d\n", __kmp_global.sched))
    K_DIAG(1, ("__kmp_global.chunk == %d\n", __kmp_global.chunk))
...
__kmp_stg_print_omp_schedule( kmp_str_buf_t * buffer, char const * name, void * data ) {
    ...
    if ( __kmp_global.chunk ) {
        switch ( __kmp_global.sched ) {
            case kmp_sch_dynamic_chunked:
                __kmp_str_buf_print( buffer, "%s,%d'\n", "dynamic", __kmp_global.chunk);
                break;
            case kmp_sch_guided_iterative_chunked:
            case kmp_sch_guided_analytical_chunked:
                __kmp_str_buf_print( buffer, "%s,%d'\n", "guided", __kmp_global.chunk);
                break;
            case kmp_sch_trapezoidal:
                __kmp_str_buf_print( buffer, "%s,%d'\n", "trapezoidal", __kmp_global.chunk);
                break;
            case kmp_sch_static:
            case kmp_sch_static_chunked:
            case kmp_sch_static_balanced:
            case kmp_sch_static_greedy:
                __kmp_str_buf_print( buffer, "%s,%d'\n", "static", __kmp_global.chunk);
                break;
            case kmp_sch_static_steal:
                __kmp_str_buf_print( buffer, "%s,%d'\n", "static_steal", __kmp_global.chunk);
                break;
            case kmp_sch_auto:
                __kmp_str_buf_print( buffer, "%s,%d'\n", "auto", __kmp_global.chunk);
                break;
        }
    } else {
        switch ( __kmp_global.sched ) {
...
__kmp_stg_parse_consistency_check( char const * name, char const * value, void * data ) {
    if ( ! __kmp_strcasecmp_with_sentinel( "all", value, 0 ) ) {
        ...
        __kmp_global.env_consistency_check = TRUE;
    } else if ( ! __kmp_strcasecmp_with_sentinel( "none", value, 0 ) ) {
        __kmp_global.env_consistency_check = FALSE;
    } else {
        KMP_WARNING( StgInvalidValue, name, value );
    }; // if
...
__kmp_stg_print_malloc_pool_incr( kmp_str_buf_t * buffer, char const * name, void * data ) {
       __kmp_stg_print_size( buffer, name, __kmp_global.malloc_pool_incr );
...
///        if( __kmp_global.cpuinfo.rtm ) { // ??? Is cpuinfo available here?
...
#define CHECK_DELIM(_x)   (*(_x) == ',' || *(_x) == 'x')
    ...
    if (*next >= '0' && *next <= '9') {
        ...
        if (*next == 's' || *next == 'S') {  // e.g. "2s"
            __kmp_global.place_num_sockets = num;
            ...
            if (*next == '@') { // socket offset, e.g. "2s@4"
                ...
                SKIP_DIGITS(next);
                num = __kmp_str_to_int(prev, *next);
                __kmp_global.place_socket_offset = num;
            }
        } else if (*next == 'c' || *next == 'C') {
            __kmp_global.place_num_cores = num;
            ...
            if (*next == '@') { // core offset, e.g. "2c@6"
                ...
                SKIP_DIGITS(next);
                num = __kmp_str_to_int(prev, *next);
                __kmp_global.place_core_offset = num;
            }
        } else if (CHECK_DELIM(next)) {
            __kmp_global.place_num_cores = num; // no letter-designator - num cores
            flagS = flagC = 1; // sockets were not specified - use default
            next++;
        } else if (*next == 't' || *next == 'T') {
            __kmp_global.place_num_threads_per_core = num;
...
__kmp_stg_print_place_threads( kmp_str_buf_t * buffer, char const * name, void * data ) {
    if (__kmp_global.place_num_sockets + __kmp_global.place_num_cores + __kmp_global.place_num_threads_per_core) {
        int comma = 0;
        kmp_str_buf_t buf;
        __kmp_str_buf_init(&buf);
        if(__kmp_env_format)
            ...
            __kmp_str_buf_print(buffer, "   %s='", name);
        if (__kmp_global.place_num_sockets) {
            __kmp_str_buf_print(&buf, "%ds", __kmp_global.place_num_sockets);
            if (__kmp_global.place_socket_offset)
                __kmp_str_buf_print(&buf, "@%d", __kmp_global.place_socket_offset);
            comma = 1;
        }
        if (__kmp_global.place_num_cores) {
            __kmp_str_buf_print(&buf, "%s%dc", comma?",":"", __kmp_global.place_num_cores);
            if (__kmp_global.place_core_offset)
                __kmp_str_buf_print(&buf, "@%d", __kmp_global.place_core_offset);
            comma = 1;
        }
        if (__kmp_global.place_num_threads_per_core)
            __kmp_str_buf_print(&buf, "%s%dt", comma?",":"", __kmp_global.place_num_threads_per_core);
        __kmp_str_buf_print(buffer, "%s'\n", buf.str );
        __kmp_str_buf_free(&buf);
/*
    } else {
...
__kmp_stg_parse_omp_display_env( char const * name, char const * value, void * data )
{
    if ( __kmp_str_match( "VERBOSE", 1, value ) )
    {
        __kmp_global.display_env_verbose = TRUE;
    } else {
        __kmp_stg_parse_bool( name, value, & __kmp_global.display_env );
    }
...
{
    if ( __kmp_global.display_env_verbose )
    {
        __kmp_stg_print_str( buffer, name, "VERBOSE" );
    } else {
        __kmp_stg_print_bool( buffer, name, __kmp_global.display_env );
    }
...
__kmp_stg_parse_omp_cancellation( char const * name, char const * value, void * data ) {
    ...
    __kmp_stg_parse_bool( name, value, & __kmp_global.omp_cancellation );
} // __kmp_stg_parse_omp_cancellation

static void
__kmp_stg_print_omp_cancellation( kmp_str_buf_t * buffer, char const * name, void * data ) {
    __kmp_stg_print_bool( buffer, name, __kmp_global.omp_cancellation );
...
__kmp_aux_env_initialize( kmp_env_blk_t* block ) {
    ...
    if ( value ) {
        ompc_set_num_threads( __kmp_global.dflt_team_nth );
    }

    /* OMP_NESTED */
    value = __kmp_env_blk_var( block, "OMP_NESTED" );
    if ( value ) {
        ompc_set_nested( __kmp_global.dflt_nested );
    }
...
# undef FIND
    ...
    if ( __kmp_global.nested_proc_bind.bind_types == NULL ) {
        ...
        __kmp_global.nested_proc_bind.size = 1;
        __kmp_global.nested_proc_bind.used = 1;
# if KMP_AFFINITY_SUPPORTED
        __kmp_global.nested_proc_bind.bind_types[0] = proc_bind_default;
# else
...
///            __kmp_global.nested_proc_bind.bind_types[0] = proc_bind_false;
///        }
///        else if ( __kmp_global.nested_proc_bind.bind_types[0] == proc_bind_true ) {
///            //
///            // OMP_PROC_BIND=true maps to OMP_PROC_BIND=spread.
///            //
///            __kmp_global.nested_proc_bind.bind_types[0] = proc_bind_spread;
///        }
///# endif /* OMP_40_ENABLED */
///
///        if ( KMP_AFFINITY_CAPABLE() ) {
...
///                }
///# if OMP_40_ENABLED
///                if ( ( __kmp_global.nested_proc_bind.bind_types[0] != proc_bind_intel )
///                  && ( __kmp_global.nested_proc_bind.bind_types[0] != proc_bind_default ) ) {
///                    if ( __kmp_affinity_type == affinity_default ) {
///                        __kmp_affinity_type = affinity_compact;
///                        __kmp_affinity_dups = FALSE;
...
///# if OMP_40_ENABLED
///        KMP_DEBUG_ASSERT( __kmp_global.nested_proc_bind.bind_types[0] != proc_bind_default );
///# endif
///    }
///
///#endif /* KMP_AFFINITY_SUPPORTED */
...
__kmp_env_print_2() {
    ...
    for ( int i = 0; i < __kmp_stg_count; ++ i ) {
        if (  __kmp_stg_table[ i ].print != NULL &&
              ( ( __kmp_global.display_env && strncmp( __kmp_stg_table[ i ].name, "OMP_", 4 ) == 0 ) || __kmp_global.display_env_verbose ) ) {
            __kmp_stg_table[ i ].print( & buffer, __kmp_stg_table[ i ].name, __kmp_stg_table[ i ].data );
        }; // if
    }; // for
