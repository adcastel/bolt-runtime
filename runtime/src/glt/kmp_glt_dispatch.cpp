/*
 * kmp_abt_dispatch.cpp: dynamic scheduling - iteration initialization and dispatch.
 */
 ...
 *
 * NOTE: __kmp_global.nth is a constant inside of any dispatch loop, however
 *       it may change values between parallel regions.  __kmp_max_nth
 *       is the largest value __kmp_global.nth may take, 1 is the smallest.
 *
 */
...
__kmp_dispatch_deo_error( int *gtid_ref, int *cid_ref, ident_t *loc_ref )
{
    ...
    if ( __kmp_global.env_consistency_check ) {
        th = __kmp_global.threads[*gtid_ref];
        ...
        lower = pr->u.p.ordered_lower;
            ...
            if ( __kmp_global.env_consistency_check ) {
                if ( pr->ordered_bumped ) {
                    struct cons_header *p = __kmp_global.threads[ gtid ]->th.th_cons;
...
__kmp_dispatch_init(
    ...
    if ( schedule == kmp_sch_static ) {
        schedule = __kmp_global.sched_static;
    } else {
        if ( schedule == kmp_sch_runtime ) {
            ...
            if ( schedule == kmp_sch_guided_chunked ) {
                schedule = __kmp_global.sched_guided;
        ...
        if ( schedule == kmp_sch_auto ) {
            ...
            schedule = __kmp_global.sched_auto;
            #ifdef KMP_DEBUG
            {
                const char * buff;
                // create format specifiers before the debug output
    ...
    if (schedule == __kmp_global.sched_static)
    {
...
__kmp_dist_get_bounds(
    ...
    if( trip_count <= nteams ) {
        ...
        if( __kmp_global.sched_static == kmp_sch_static_balanced ) {
            ...
            *pupper = *plower + chunk * incr - ( team_id < extras ? 0 : incr );
            ...
            KMP_DEBUG_ASSERT( __kmp_global.sched_static == kmp_sch_static_greedy );
                // Unknown static scheduling type.
