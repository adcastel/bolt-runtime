/*
 * kmp_abt_threadprivate.c -- OpenMP threadprivate support library
 */
...
{
    if( ! TCR_4(__kmp_global.init_common) ) {
        int q;
#ifdef KMP_DEBUG
        int gtid;
#endif
        ...
        for(gtid = 0 ; gtid < __kmp_global.threads_capacity; gtid++ )
            if( __kmp_global.root[gtid] ) {
                KMP_DEBUG_ASSERT( __kmp_global.root[gtid]->r.r_uber_thread );
                for ( q = 0; q< KMP_HASH_TABLE_SIZE; ++q)
                    KMP_DEBUG_ASSERT( !__kmp_global.root[gtid]->r.r_uber_thread->th.th_pri_common->data[q] );
/*                    __kmp_global.root[ gitd ]-> r.r_uber_thread -> th.th_pri_common -> data[ q ] = 0;*/
            }
#endif /* KMP_DEBUG */
        ...
        TCW_4(__kmp_global.init_common, TRUE);
    }
}
...
__kmp_common_destroy( void )
{
    if( TCR_4(__kmp_global.init_common) ) {
        ...
        TCW_4(__kmp_global.init_common, FALSE);
        ...
        for (q = 0; q < KMP_HASH_TABLE_SIZE; ++q) {
            ...
            for (d_tn = __kmp_threadprivate_d_table.data[ q ]; d_tn; d_tn = d_tn->next) {
                if (d_tn->is_vec) {
                    if (d_tn->dt.dtorv != 0) {
                        for (gtid = 0; gtid < __kmp_global.all_nth; ++gtid) {
                            if( __kmp_global.threads[gtid] ) {
                                if( (__kmp_global.foreign_tp) ? (! KMP_INITIAL_GTID (gtid)) :
                                                         (! KMP_UBER_GTID (gtid)) ) {
                                    tn = __kmp_threadprivate_find_task_common( __kmp_global.threads[ gtid ]->th.th_pri_common,
                                                                               gtid, d_tn->gbl_addr );
                                    if (tn) {
...
__kmp_common_destroy_gtid( int gtid )
{
    ...
    if( (__kmp_global.foreign_tp) ? (! KMP_INITIAL_GTID (gtid)) :
        ...
        if( TCR_4(__kmp_global.init_common) ) {
            ...
            for (tn = __kmp_global.threads[ gtid ]->th.th_pri_head; tn; tn = tn->link) {
...
dump_list( void )
{
    ...
    for (p = 0; p < __kmp_global.all_nth; ++p) {
        if( !__kmp_global.threads[p] ) continue;
        for (q = 0; q < KMP_HASH_TABLE_SIZE; ++q) {
            if (__kmp_global.threads[ p ]->th.th_pri_common->data[ q ]) {
                struct private_common *tn;

                KC_TRACE( 10, ( "\tdump_list: gtid:%d addresses\n", p ) );

                for (tn = __kmp_global.threads[ p ]->th.th_pri_common->data[ q ]; tn; tn = tn->next)                 {
...
kmp_threadprivate_insert_private_data( int gtid, void *pc_addr, void *data_addr, size_t pc_size )
{
    ...
    KMP_DEBUG_ASSERT( __kmp_global.threads[ gtid ] &&
            __kmp_global.threads[ gtid ] -> th.th_root -> r.r_active == 0 );
...
*/
        ...
        __kmp_acquire_lock( &__kmp_global.global_lock, gtid );
        ...
        *lnk_tn = d_tn;
        ...
        __kmp_release_lock( &__kmp_global.global_lock, gtid );
    }
}
...
*/
    ...
    if ( (__kmp_global.foreign_tp) ? (KMP_INITIAL_GTID (gtid)) : (KMP_UBER_GTID (gtid)) ) {
    ...
    tt = &(__kmp_global.threads[ gtid ]->th.th_pri_common->data[ KMP_HASH(pc_addr) ]);
    ...
    *tt = tn;
    ...
    tn->link = __kmp_global.threads[ gtid ]->th.th_pri_head;
    __kmp_global.threads[ gtid ]->th.th_pri_head = tn;
...
__kmpc_threadprivate(ident_t *loc, kmp_int32 global_tid, void *data, size_t size)
{
    ...
    if ( ! __kmp_global.threads[global_tid] -> th.th_root -> r.r_active && ! __kmp_global.foreign_tp ) {
        ...
        KC_TRACE( 50, ("__kmpc_threadprivate: T#%d try to find private data at address %p\n",
                       global_tid, data ) );
        tn = __kmp_threadprivate_find_task_common( __kmp_global.threads[ global_tid ]->th.th_pri_common, global_tid, data );
...
__kmpc_threadprivate_cached(
    ...
    if ( TCR_PTR(*cache) == 0) {
        __kmp_acquire_lock( & __kmp_global.global_lock, global_tid );

        if ( TCR_PTR(*cache) == 0) {
            __kmp_acquire_bootstrap_lock(&__kmp_global.tp_cached_lock);
            __kmp_global.tp_cached = 1;
            __kmp_release_bootstrap_lock(&__kmp_global.tp_cached_lock);
            void ** my_cache;
            my_cache = (void**)
                __kmp_allocate(sizeof( void * ) * __kmp_global.tp_capacity + sizeof ( kmp_cached_addr_t ));
            ...
            /* TODO: free all this memory in __kmp_common_destroy using __kmp_global.threadpriv_cache_list */
            /* Add address of mycache to linked list for cleanup later  */
            kmp_cached_addr_t *tp_cache_addr;

            tp_cache_addr = (kmp_cached_addr_t *) & my_cache[__kmp_global.tp_capacity];
            tp_cache_addr -> addr = my_cache;
            tp_cache_addr -> next = __kmp_global.threadpriv_cache_list;
            __kmp_global.threadpriv_cache_list = tp_cache_addr;
        ...
        __kmp_release_lock( & __kmp_global.global_lock, global_tid );
    }
