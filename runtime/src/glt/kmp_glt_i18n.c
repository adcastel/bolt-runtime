...
#include "kmp_glt_debug.h"
#include "kmp_glt.h"
//#include "kmp_lock.h"
#include "kmp_glt_io.h"          // __kmp_printf.
...
__kmp_i18n_catopen(
    ...
    if ( status == KMP_I18N_CLOSED ) {
        __kmp_acquire_bootstrap_lock( &__kmp_global.cat_lock );
        if ( status == KMP_I18N_CLOSED ) {
            __kmp_i18n_do_catopen();
        }; // if
        __kmp_release_bootstrap_lock( &__kmp_global.cat_lock );
    }; // if
...
__kmp_i18n_do_catopen(
    ...
    if ( status == KMP_I18N_ABSENT ) {
      if (__kmp_global.generate_warnings > kmp_warnings_low) { // AC: only issue warning in case explicitly asked to
	...
	KMP_INFORM( WillUseDefaultMessages );
        ...
        if ( strcmp( version.str, expected ) != 0 ) {
            ...
            } // __kmp_global.generate_warnings
        }; // if
    ...
    */
	...
	KMP_INFORM( WillUseDefaultMessages );
        ...
        if ( strcmp( version.str, expected ) != 0 ) {
            ...
            __kmp_i18n_catclose();
            status = KMP_I18N_ABSENT;  // And mark it as absent.
            if (__kmp_global.generate_warnings > kmp_warnings_low) {
                // And now print a warning using default messages.
                __kmp_msg(
                    kmp_ms_warning,
...
__kmp_msg(
    ...
    if ( severity != kmp_ms_fatal && __kmp_global.generate_warnings == kmp_warnings_off )
