/*
 * kmp_abt_tasking.c -- OpenMP 3.0 tasking support.
 */
...
//    task->td_parent   = NULL;  // fix for CQ230101 (broken parent task info in debugger)
    ...
    task->td_flags.tasking_ser = ( __kmp_global.tasking_mode == tskm_immediate_exec );
                  ...
                  kmp_routine_entry_t task_entry )
{
    ...
    taskdata->td_flags.tasking_ser = ( __kmp_global.tasking_mode == tskm_immediate_exec );
...
__kmp_invoke_task( kmp_int32 gtid, kmp_task_t *task, kmp_taskdata_t * current_task )
{
    ...
    if (__kmp_global.omp_cancellation) {
...
///                                   USE_ITT_BUILD_ARG(itt_sync_obj), __kmp_global.task_stealing_constraint );
///            }
        }
...
/*
    ...
    if ( __kmp_global.tasking_mode != tskm_immediate_exec && __kmp_global.init_parallel ) {
        // GEH TODO: shouldn't we have some sort of OMPRAP API calls here to mark begin wait?
...
*/
    ...
    KA_TRACE(10, ("__kmpc_omp_taskyield(exit): T#%d task %p resuming, "
                  "returning TASK_CURRENT_NOT_QUEUED\n",
                  __kmp_gtid_from_thread(thread), taskdata) );
                  ...
                  kmp_int32 is_constrained )
{
    ...
    if ( !is_constrained ) {
        ...
        // we need to check if the candidate obeys task scheduling constraint:
        // only child of current task can be scheduled
        kmp_taskdata_t * current = __kmp_global.threads[ gtid ]->th.th_current_task;
        kmp_int32        level = current->td_level;
        kmp_taskdata_t * parent = taskdata->td_parent;
        while ( parent != current && parent->td_level > level ) {
            parent = parent->td_parent;  // check generation up to the level of the current task
                                               ...
                                               USE_ITT_BUILD_ARG(void * itt_sync_obj), kmp_int32 is_constrained)
{
    ...
    KMP_DEBUG_ASSERT( __kmp_global.tasking_mode != tskm_immediate_exec );
    KMP_DEBUG_ASSERT( thread == __kmp_global.threads[ gtid ] );
    ...
    while (( task = __kmp_remove_my_task( thread, gtid, task_team, is_constrained )) != NULL ) {
        ...
        KMP_YIELD( __kmp_global.library == library_throughput );   // Yield before executing next task
    }
    ...
    }
    {
        ...
        if ( ( __kmp_global.tasking_mode == tskm_task_teams ) &&
...
///static kmp_bootstrap_lock_t __kmp_global.task_team_lock = KMP_BOOTSTRAP_LOCK_INITIALIZER( __kmp_global.task_team_lock );
...
__kmp_allocate_task_team( kmp_info_t *thread, kmp_team_t *team )
{
    ...
    if (TCR_PTR(__kmp_free_task_teams) != NULL) {
        ...
        __kmp_acquire_bootstrap_lock( &__kmp_global.task_team_lock );
        ...
        __kmp_release_bootstrap_lock( &__kmp_global.task_team_lock );
    }
...
__kmp_wait_to_unref_task_teams(void)
{
    ...
    for (;;) {
        ...
        //             this routine.
        for (thread = (kmp_info_t *)__kmp_global.thread_pool;
             thread != NULL;
...
///            }
        ...
        KMP_YIELD( TCR_4(__kmp_global.nth) > __kmp_global.avail_proc );
...
// This routine may only called when __kmp_global.tasking_mode == tskm_extra_barrier.
...
__kmp_tasking_barrier( kmp_team_t *team, kmp_info_t *thread, int gtid )
{
    ...
    int flag = FALSE;
    KMP_DEBUG_ASSERT( __kmp_global.tasking_mode == tskm_extra_barrier );
